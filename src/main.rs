...
use s3_bucket_service::download_file_from_s3;
...
async fn main() -> Result<(), s3::Error> {
    ...
    let state = initialize_client(&instructions.s3, credentials.s3).await;

    let jar = download_file_from_s3(
        state.aws_client,
    ...
    let command = run(
        jar,
        &instructions.boot.file_name,
        &instructions.boot.boot_command
    ).await
        .expect("Unable to run jar file.");

    let mut commands = HashMap::new();
    commands.insert(&instructions.boot.file_name, command);
